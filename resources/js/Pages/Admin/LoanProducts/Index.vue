...
const submit = () => {
    form.post(route('admin.loan-products.store'), {
        onSuccess: () => {
...
<template>
    <Head title="Admin - Loan Products" />
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    >
                        <div class="p-6">
                            <div class="flex items-center justify-between">
                                ...
                                >
                                    {{ product.interest_type === 'flat' ? 'Flat' : 'Reducing' }}
                                </span>
                            </div>
                            ...
                            <div class="mt-4 space-y-2 text-sm text-gray-600">
                                <div class="flex justify-between">
                                    ...
                                    <span class="font-semibold text-gray-900">{{ product.interest_rate }}%</span>
                                </div>
                                <div class="flex justify-between">
                                    <span>Term Length:</span>
                                    <span class="font-semibold text-gray-900">{{ product.term_length }} {{ product.term_unit }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span>Processing Fee:</span>
                                    <span class="font-semibold text-gray-900">KES {{ parseFloat(product.processing_fee).toLocaleString(undefined, {minimumFractionDigits: 2}) }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="bg-gray-50 px-6 py-4 border-t border-gray-100 text-xs text-gray-500">
                    ...
                    <div v-if="products.length === 0" class="col-span-full bg-white rounded-lg p-12 text-center text-gray-500 border border-gray-100 shadow-sm">
                        No loan products configured. Click "Create Product" to add.
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
                    <form @submit.prevent="submit">
                            ...
                            <div class="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        v-model="form.name"
                                        id="name"
                                        placeholder="e.g. SME Business Expansion"
                                        required
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <label for="interest_type" class="block text-sm font-medium text-gray-700">Interest Mode</label>
                                        <select
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                            ...
                            >
                                {{ form.processing ? 'Creating...' : 'Save Product' }}
                            </button>
                            <button
                                type="button"
