...
const handleApprove = () => {
    approveForm.post(route('loans.approve', props.loan.id));
};

const handleDisburse = () => {
    disburseForm.post(route('loans.disburse', props.loan.id));
};
...
<template>
    ...
    <AuthenticatedLayout>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="overflow-hidden bg-white shadow rounded-lg border border-gray-100 p-6">
                            <h3 class="text-lg font-bold text-gray-900 border-b border-gray-100 pb-4 mb-4">Loan Profile</h3>
                            
                            <dl class="space-y-4 text-sm">
                                <div>
                                    <dt class="text-gray-500 font-medium">Customer</dt>
                                    <dd class="text-gray-900 font-semibold mt-1">{{ loan.customer.name }}</dd>
                                ...
                                <div>
                                    <dt class="text-gray-500 font-medium">Product Config</dt>
                                    <dd class="text-gray-900 font-semibold mt-1">{{ loan.loan_product.name }}</dd>
                                    <dd class="text-xs text-gray-500 mt-0.5">
                                        {{ loan.loan_product.interest_rate }}% {{ loan.loan_product.interest_type === 'flat' ? 'Flat' : 'Reducing Balance' }} / {{ loan.loan_product.term_length }} {{ loan.loan_product.term_unit }}
                                    </dd>
                                </div>
                                ...
                                <div>
                                    <dt class="text-gray-500 font-medium">Interest Charge</dt>
                                ...
                                <div>
                                    <dt class="text-gray-500 font-medium">Total Repayable</dt>
                                ...
                                <div class="border-t border-gray-100 pt-4">
                                    <dt class="text-indigo-600 font-bold">Outstanding Balance</dt>
                                    <dd class="text-2xl font-black text-indigo-900 mt-1">KES {{ parseFloat(loan.balance).toLocaleString(undefined, {minimumFractionDigits: 2}) }}</dd>
                                </div>
                            </dl>
                        </div>

                        <!-- Administrative Workflows Panel -->
                        <div class="overflow-hidden bg-white shadow rounded-lg border border-gray-100 p-6">
                            <h3 class="text-lg font-bold text-gray-900 border-b border-gray-100 pb-4 mb-4">Operations Control</h3>
                            
                            <div class="space-y-4">
                                <!-- Approve trigger -->
                                <div v-if="loan.status === 'pending'">
                                    <p class="text-xs text-gray-500 mb-2">Approve the application to verify and set it ready for B2C disbursement payout.</p>
                                    <button
                                        @click="handleApprove"
                                    ...
                                    >
                                        {{ approveForm.processing ? 'Processing...' : 'Approve Application' }}
                                    </button>
                                </div>
                                ...
                                <div v-if="loan.status === 'active'">
                                    <p class="text-xs text-gray-500 mb-2">Request mobile payment by initiating M-Pesa STK push directly onto customer's handset.</p>
                                    <button
                                    ...
                                    >
                                        Repay (M-Pesa STK Push)
                                    </button>
                                </div>

                                <div v-if="loan.status === 'closed'">
                                    <div class="rounded-md bg-green-50 p-4 border border-green-200 text-center">
                                        <p class="text-sm font-semibold text-green-800">Loan Fully Repaid</p>
                                        <p class="text-xs text-green-600 mt-1">This loan account is closed and resolved.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="overflow-hidden bg-gray-50 shadow rounded-lg border border-dashed border-gray-300 p-6">
                            <h3 class="text-base font-bold text-gray-900 border-b border-gray-200 pb-3 mb-4">Webhook Simulator</h3>
                                ...
                                <div v-if="loan.disbursements.some(d => d.status === 'initiated')">
                                    ...
                                    <div class="flex gap-2">
                                        <button
                                            ...
                                            class="flex-1 text-center bg-green-100 text-green-800 border border-green-300 rounded px-2 py-1.5 text-xs font-semibold hover:bg-green-200 transition"
                                        >
                                            Simulate Success
                                        </button>
                                        <button
                                            @click="triggerB2cSimulation(loan.disbursements.find(d => d.status === 'initiated').reference, 'failed')"
                                            class="flex-1 text-center bg-red-100 text-red-800 border border-red-300 rounded px-2 py-1.5 text-xs font-semibold hover:bg-red-200 transition"
                                        >
                                            Simulate Fail
                                        </button>
                                    </div>
                                ...
                                <div v-if="loan.stk_requests.some(s => s.status === 'pending')">
                                    ...
                                    <div class="grid grid-cols-3 gap-2">
                                        <button
                                            ...
                                            class="text-center bg-green-100 text-green-800 border border-green-300 rounded px-1.5 py-1 text-xs font-semibold hover:bg-green-200 transition"
                                        >
                                            Paid
                                        </button>
                                        <button
                                            @click="triggerStkSimulation(loan.stk_requests.find(s => s.status === 'pending').checkout_reference, 'mismatch')"
                                            class="text-center bg-yellow-100 text-yellow-800 border border-yellow-300 rounded px-1.5 py-1 text-xs font-semibold hover:bg-yellow-200 transition"
                                        >
                                            Mismatch
                                        </button>
                                        <button
                                            @click="triggerStkSimulation(loan.stk_requests.find(s => s.status === 'pending').checkout_reference, 'cancelled')"
                                            class="text-center bg-red-100 text-red-800 border border-red-300 rounded px-1.5 py-1 text-xs font-semibold hover:bg-red-200 transition"
                                        >
                                            Cancel
                                        </button>
                                    </div>
                    ...
                    <!-- Right: Schedule & Ledger Transactions -->
                    <div class="lg:col-span-2 space-y-6">
                            ...
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200 text-sm">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th scope="col" class="px-4 py-3 text-left font-semibold text-gray-600">No.</th>
                                            <th scope="col" class="px-4 py-3 text-left font-semibold text-gray-600">Due Date</th>
                                            <th scope="col" class="px-4 py-3 text-right font-semibold text-gray-600">Principal</th>
                                            <th scope="col" class="px-4 py-3 text-right font-semibold text-gray-600">Interest</th>
                                            <th scope="col" class="px-4 py-3 text-right font-semibold text-gray-600">Installment</th>
                                            <th scope="col" class="px-4 py-3 text-right font-semibold text-gray-600">Paid</th>
                                            <th scope="col" class="px-4 py-3 text-center font-semibold text-gray-600">Status</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-200">
                                        <tr v-for="installment in loan.installments" :key="installment.id" class="hover:bg-gray-50">
                                            <td class="px-4 py-3.5 font-medium text-gray-900">{{ installment.installment_number }}</td>
                                            <td class="px-4 py-3.5 text-gray-600">{{ new Date(installment.due_date).toLocaleDateString() }}</td>
                                            ...
                                            <td class="px-4 py-3.5 text-center">
                                                <span
                                                    ...
                                                    class="inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold uppercase"
                                                >
                                                    {{ installment.status }}
                                                </span>
                                            </td>
                        ...
                        <!-- Transaction Logs (Disbursements / STK / Payments) -->
                        <div class="overflow-hidden bg-white shadow rounded-lg border border-gray-100 p-6">
                            <h3 class="text-lg font-bold text-gray-900 border-b border-gray-100 pb-4 mb-4">Ledger Transactions</h3>
                            
                            <!-- Tabs Structure inside Show page -->
                            <div class="space-y-6">
                                
                                <!-- disbursements list -->
                                <div>
                                    <h4 class="text-sm font-bold text-gray-800 uppercase tracking-wide mb-2">B2C Payout Transactions</h4>
                                ...
                                <!-- payments list -->
                                <div>
                                    <h4 class="text-sm font-bold text-gray-800 uppercase tracking-wide mb-2">Reconciled Cash Payments</h4>
                                    <div class="overflow-x-auto border border-gray-100 rounded-lg">
                                        <table class="min-w-full divide-y divide-gray-200 text-xs">
                                            <thead class="bg-gray-50">
                                                <tr>
                                            ...
                                            <tbody class="divide-y divide-gray-100 bg-white">
                                                ...
                                                <tr v-if="loan.payments.length === 0">
                                                    <td colspan="4" class="px-3 py-4 text-center text-gray-400 italic">No repayment records.</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
        ...
        <!-- Repayment STK Modal -->
        <div v-if="isRepayModalOpen" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                ...
                <div class="inline-block transform overflow-hidden rounded-lg bg-white text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:align-middle">
                    <form @submit.prevent="handleRepaySubmit">
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                            ...
                            >
                                {{ repayForm.processing ? 'Initiating STK...' : 'Send STK Push' }}
                            </button>
                            <button
                                type="button"
