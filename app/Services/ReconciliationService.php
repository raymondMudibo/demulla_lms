...
class ReconciliationService
{
    ...
    {
        Cache::lock("stk_lock_{$checkoutReference}", 10)->block(5, function () use ($checkoutReference, $payload) {
            DB::transaction(function () use ($checkoutReference, $payload) {
                ...
                if (! $stkRequest) {
                    Log::warning("STK request not found for reference: {$checkoutReference}");

                    return;
                }

                if ($stkRequest->status !== 'pending') {
                    Log::warning("STK callback already processed for reference: {$checkoutReference}, current status: {$stkRequest->status}");

                    return;
                }

                // Pessimistic lock on the associated loan
                $loan = Loan::where('id', $stkRequest->loan_id)
                    ->lockForUpdate()
                    ->firstOrFail();
                ...
                if ($resultCode === 0) {
                    ...
                    $this->allocateLedger($payment, $loan);
    ...
    protected function allocateLedger(Payment $payment, ?Loan $lockedLoan = null): void
    {
        $loan = $lockedLoan ?? Loan::where('id', $payment->loan_id)->lockForUpdate()->firstOrFail();
